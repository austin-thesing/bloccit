class QuestionsController < ApplicationController
  ...
  def create
    ...
    if @question.save
      flash[:notice] = "Question: '#{@question.title}' was saved."
      redirect_to @question
    else
      flash[:error] = "There was an error saving your question. Please try again."
      render :new # reloads the view so there is a new form
    end
  end
  def edit
    @question = Question.find(params[:id])
  ...
  def destroy
    ...
    if @question.destroy
      ...
      flash[:error] = "An error occured while trying to delete the post. Please try again."
      render :show
    end
  end
end
